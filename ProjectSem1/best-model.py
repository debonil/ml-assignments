...
def model_accuracy(input_size=1024, hidden_sizes=[512, 64], output_size=10):
...
for i in range(n):
    for j in range(n):
        for k in range(n):
            accs.append(model_accuracy(hidden_sizes=[2**(11-i), 2**(11-j), 2**(11-k)]))
            accsLbl.append([2**(11-i), 2**(11-j), 2**(11-k)])

ij = 0 #np.argmax(np.array(accs))

print(f'\n\nBest model :: Hidden Layer = {accsLbl} Model Accuracy = {accs[ij]}')
